(ns stc_pay.main
  "Kotodama WASM entrypoint for the stc_pay clean-room actor (L4) — Clojure port.
  ...
  facts (`stc_pay.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "stc_pay")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"stc_pay.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "stc_pay-compat" :tier tier :entities entities} 200])
...
(defn start! [] :stc_pay-compat/ready)
